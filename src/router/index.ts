...
import CreateInfographicView from '../views/CreateInfographicView.vue'
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          component: CreateInfographicView
        },
        {
          path: 'space-design',
...
const generationFlowRoutePrefixes = ['/create']
...
router.beforeEach((to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
  ...
  if (isActuallyGenerating) {
    ...
    // 允许访问生成流程路由（即使正在生成中）
    if (
      generationFlowRoutes.includes(to.path) ||
      generationFlowRoutePrefixes.some(prefix => to.path.startsWith(prefix))
    ) {
      // 隐藏导航守卫提示（如果正在显示）
      store.hideNavigationGuardModal()
      next()
